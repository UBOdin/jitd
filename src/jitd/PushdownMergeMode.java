...
public class PushdownMergeMode extends Mode
{
  ...
  public static final int BLOCK_SIZE = 10*1000*1000;
  ...
  public Pair<KeyValueIterator, Cog> mergePartitions(Cog cog, long low, long high)
  {
    ...
    long partitionMergeMin = Long.MAX_VALUE, partitionMergeMax = Long.MIN_VALUE;
    ...
    for(int i = 1; i < partitions.size(); i++){
      ...
      if(components.iter != null) { 
        hasMergeWork = true; 
        partitionMergeMin = Math.min(partitionMergeMin, components.lowKey);
        partitionMergeMax = Math.max(partitionMergeMax, components.highKey);
      }
    ...
    if(hasMergeWork){
      ...
      ExtractedComponents root =
        extractPartitions(partitions.get(0), partitionMergeMin, partitionMergeMax, true);
