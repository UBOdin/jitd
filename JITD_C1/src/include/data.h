...
/** The functions and structures in this file follow a few 
 *  conventions that may be nonstandard.  Please consult this 
 *  list before defining new functions or changing anything

 *   - Function and structure names use under_scores and not camelCase.
 *   - Structures are defined in typedefs.  By convention `struct foo` 
 *     represents the actual structure, while `foo` represents a 
...
/*********************** BUFFERS ***********************/
/** A header for a reference-counted array of records.  
 *  Due to the way C handles structure memory allocation, 
 *  simply prepending this structure to the memory
 *  allocated for the array allows the data[] field to 
 *  represent the buffer. In other words, you can allocate a buffer as:
 
 *  malloc(sizeof(struct buffer) + (NUM_RECORDS * sizeof(struct record)));

 *  and free the entire thing with one operation.
 *  Memory management for buffers is abstracted for your 
 *  convenience in the buffer_* methods below.
 */
...
/*********************** ITERATORS ***********************/
/** A C "interface" definition for iterator objects.  Instances 
 *  of iterator_impl provide method definitions for the primary 
 *  operations on the iterator. Instances of struct iterator are 
 *  actual object instances.  Use the iterator_* methods to access 
 *  instances.
 */
 ...
 * You should use these methods rather than calling the class-specific 
 * versions. That said, each iterator class has a class-specific 
 * constructor. See `array_scan` below for an example.
 */
...
/* iter_next(iter, r):
 *   -Read the next record from the iterator.  Results are undefined 
 *    if iter_has_next indicates that there are no more records to read.
...
/* iter_concat(a, b):
 *   -Create an iterator that returns all of the records in `a` 
 *    and then returns all of the records in `b`
...
/***********************  BUFFER METHODS ***********************/
/* These methods operate over the buffer interface defined above.  
 * You should use `buffer_alloc`, `buffer_retain`, and `buffer_release` 
 * instead of native memory management (i.e., malloc/free)
 */

/* buffer_alloc(size):
 *   -Allocate a buffer containing `size` records.  The reference count 
 *    of the buffer will be initialized to 1.
...
/* buffer_release(b):
 *   -Decrement the buffer's reference count and free it if 
 *    the count reaches zero. Analogous to `free`, but reference-counted.
...
/* buffer_key(b, i):
 *   -Utility method for looking up the key field of the record 
...
/* buffer_value(b, i):
 *   -Utility method for looking up the value field of the 
 *    record at a specified index.
 * buffer b: The buffer to reference into
 * int idx: The index of the record to return a pointer to
 * returns long: The key of record `idx` of buffer `b`*/
...
/* record_copy(src, dst):
 *   -Utility method for copying records.  The memory pointed to 
 *    by `dst` is overwritten by the contents pointed to by `src`
...
/* record_swap(ir, jr):
 *   -Utility method for swapping records.  After the call, the 
 *    contents of the records pointed to by `ir` and `jr` 
 *    will be swapped.
 ...
 *   -Search for `key` in records from (inclusive) `r[start]` to 
 *    (exclusive) `r[start+len]`.  `r` must be sorted by the record 
 *    keys.  If `r` contains at least one instance of `key`, the 
 *    index of one such record will be returned.  If `r` does not 
 *    contain `key` the highest index of a record with a key lower 
 *    than `key` is returned instead.
 ...
 *   -Create an iterator over records in `buffer` with keys in 
 *    the range `[low, high)` and indexes in the range `[start, start+len)`.  
 *    `buffer` must be sorted: Complexity will be O(|records returned|)
