...
let mk_program () = { cogs = ref []; rules = ref []; }
let add_cog  (p:program_t) (cog: cog_t)  = p.cogs  := cog  :: !(p.cogs ); p
let add_rule (p:program_t) (rule:rule_t) = p.rules := rule :: !(p.rules); p
...
let rec string_of_stmt ?(prefix="")= 
  let rcr = string_of_stmt ~prefix:(prefix^"  ") in function
  | Apply(rule, tgt) -> 
...
and string_of_match_list ?(prefix="") (pats:((pattern_t * stmt_t) list)) = 
  let rcr = string_of_stmt ~prefix:(prefix^"  ") in
    (String.concat "" (List.map (fun (pat, ex) ->
      prefix^"| "^(string_of_pattern pat)^"\n"^(rcr ex)^"\n"
    ) pats))
and string_of_rule_ref = function
  | RuleRef(r, params) -> r^(String.concat "," (List.map string_of_rule_ref params))
  | RuleParam(ex)      -> ex
...
let string_of_program (prog:program_t) =
  ...
  (String.concat "\n\n" (List.map string_of_rule !(prog.rules)))
